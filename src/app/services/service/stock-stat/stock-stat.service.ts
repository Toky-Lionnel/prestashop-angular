...
export class StockStatService {
  ...
  private extractNumeric(value: any): number | null {
    ...
    async getReservedProducts(): Promise<Array<{ id_product: number; id_product_attribute: number; quantity: number ; category : number}>> {
      try {
    ...
    async getStockAvailableGrouped(): Promise<StockAvailableGrouped[]> {
      try {
        ...
        for (const stock of stocks) {
          ...
          const product = await this.productService.getProductDetailById(idProduct).catch(() => null);
    ...
    async getPhysicalQuantityGrouped(): Promise<PhysicalQuantityGrouped[]> {
      try {
        ...
        const reserved = await this.getReservedProducts();
    ...
    async getCategoryStockSummary(): Promise<CategoryStockSummary[]> {
      try {
        ...
        for (const [pid, data] of productMap) {
          ...
          if (!existing.representativeProductId) existing.representativeProductId = pid;
        ...
        for (const [catId, sums] of categoryMap) {
          ...
          // Essayer d'obtenir un nom de catégorie à partir d'un produit représentatif
          if (sums.representativeProductId) {
            const prod = await this.productService.getProductDetailById(sums.representativeProductId).catch(() => null);
            if (prod && Array.isArray(prod.categories)) {
              const catObj = prod.categories.find((c: any) => c.id === catId);
